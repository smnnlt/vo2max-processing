...
# Remove Douglas bag measurements
ext <- ext[ext$type != "douglas", ]
...
ext[ext$ptype == "time" & !is.na(ext$ptype), ] |>
...
ggsave("../plots/duration_count.png", width = 5, height = 3, dpi = 600, bg = "white")
```
...
ggplot(ext_count, aes(area = n, fill = proc_combination, label = label, subgroup = subgroup)) +
  geom_treemap(show.legend = FALSE, start = "topleft") +
  geom_treemap_text(reflow = TRUE, place = "centre", start = "topleft")

ggsave("../plots/strategies.png", width = 5, height = 3, dpi = 600, bg = "white")
```
